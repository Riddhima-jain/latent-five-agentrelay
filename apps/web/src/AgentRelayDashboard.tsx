...
type WorkflowScenario = "normal" | "timeout" | "denial" | "resource_scope_breach" | "bypass_protection" | "evidence_acceptance" | "duplicate_approval";
  ...
  return (
    <section className="middleware-interventions" aria-labelledby="middleware-interventions-title">
      ...
      <div className="intervention-grid">
        ...
        {idempotency && <article className="intervention-card idempotent"><span className="intervention-icon">⇉</span><div><header><strong>Duplicate approvals deduplicated</strong><span>Exactly-once execution</span></header><p><b>{idempotency.concurrentRequests}</b> concurrent executions raced the atomic idempotency ledger for one approval.</p><dl><div><dt>Ledger claim won</dt><dd>{idempotency.claimsWon}</dd></div><div><dt>Duplicates rejected</dt><dd>{idempotency.duplicatesRejected}</dd></div><div><dt>Real emails sent</dt><dd>{idempotency.sends}</dd></div></dl></div></article>}
      </div>
    </section>
  );
}
