...
export function assessEvidence(
  ...
  } else if (record.sourceRefs.some((sourceRef) => !authorizedSourceRefs.has(sourceRef))) {
    reasons.push("Evidence source was not obtained through an authorized resource read");
  }
  return reasons.length === 0 ? { status: "accepted", reasons: [] } : { status: "rejected", reasons };
}

export function applyEvidenceAcceptance(record: EvidenceRecord, producerTask: AgentTask, authorizedSourceRefs: ReadonlySet<string>): EvidenceRecord {
  const acceptance = assessEvidence(record, producerTask, authorizedSourceRefs);
