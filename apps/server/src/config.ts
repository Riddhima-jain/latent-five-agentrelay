...
export function isModelConfigured(config: AppConfig): boolean {
  if (config.modelProvider === "ark") return isArkConfigured(config);
  return (
    config.geminiApiKey.length > 0 &&
    !config.geminiApiKey.startsWith("replace-") &&
    config.geminiModel.length > 0 &&
    !config.geminiModel.includes("replace-")
  );
}

function geminiProxyBaseUrl(config: AppConfig): string {
  if (config.geminiProxyBaseUrl) return config.geminiProxyBaseUrl;
  const host = config.runtimeProvider === "container" ? "host.docker.internal" : "127.0.0.1";
  return "http://" + host + ":" + config.port + "/model/v1";
}

export async function writeCodexConfig(config: AppConfig): Promise<void> {
  await mkdir(config.codexHome, { recursive: true });
  const toml = config.modelProvider === "gemini"
    ? [
      "# Generated by Volc Agent Launchpad. Edit environment variables, not this file.",
      "model = " + JSON.stringify(config.geminiModel),
      'model_provider = "gemini_proxy"',
      "",
      "[model_providers.gemini_proxy]",
      'name = "Gemini Responses bridge"',
      "base_url = " + JSON.stringify(geminiProxyBaseUrl(config)),
